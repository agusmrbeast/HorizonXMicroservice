...
# Ensure .env file exists by copying from development if needed
env_file = project_root / ".env"
env_dev = project_root / "envs" / ".env.development"
if not env_file.exists() and env_dev.exists():
    import shutil
    print(f"Copying environment file from {env_dev} to {env_file}")
    shutil.copy2(env_dev, env_file)

# Load environment variables
from dotenv import load_dotenv
load_dotenv(dotenv_path=env_file, override=True)

# Print current working directory and environment status for debugging
