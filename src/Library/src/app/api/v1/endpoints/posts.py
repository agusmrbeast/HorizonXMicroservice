...
from src.app.schemas import Post, PostCreate, PostUpdate, UserDict
...
async def create_post(
    ...
    current_user: UserDict = Depends(has_permission("posts", "create")),
) -> Post:
    """Create new post."""
    post_service = PostService(db)
    return await post_service.create(post_in, current_user["id"])
...
async def user_feed(
    ...
    post_service = PostService(db)
    return await post_service.get_user_feed(current_user["id"])
...
async def get_post(
    post_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: UserDict = Depends(has_permission("posts", "read")),
) -> Post:
    """Get post by ID."""
    post_service = PostService(db)
...
async def update_post(
    ...
    post_in: PostUpdate,
    db: AsyncSession = Depends(get_db),
    current_user: UserDict = Depends(has_permission("posts", "update")),
) -> Post:
    """Update post."""
    post_service = PostService(db)
    post = await post_service.get(post_id)
...
async def delete_post(
    post_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: UserDict = Depends(has_permission("posts", "delete")),
) -> None:
    """Delete post."""
    post_service = PostService(db)
    post = await post_service.get(post_id)
